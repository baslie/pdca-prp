...
- **`.trainer-clients`** — полоса клиентов в секции `#about-trainer`: uppercase,
  статичный кегль 0.875rem (= `.t-meta`), letter-spacing 0.08em, line-height 2.
  Разрядка вне токенов (`tracking-label` 0.32em здесь слишком широк) — это «лента
  логотипов», плакатный элемент, а не текст.
