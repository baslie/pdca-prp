...
Этот документ — единственный источник правды по **макротипографике**: шкале
кеглей, цветовым ролям, весам и tracking. Любая правка HTML/JS, добавление
нового блока или редизайн секции опирается на него.
Перед коммитом — пройдись по [чек-листу](#7-чек-лист-перед-коммитом).

**Микротипографика** (неразрывные пробелы, тире, кавычки, многоточия) живёт
отдельно и расставляется автоматически на сборке — см.
[раздел 9](#9-микротипографика-типограф) и `CLAUDE.md`.
...
Плагин [`astro-typograf`](https://github.com/mishamyrt/astro-typograf) на хуке
`astro:build:done` проходит по собранному `dist/*.html`, находит элементы
по CSS-селектору и типографирует их содержимое. Конфигурация целиком —
в [`astro-typograf.config.mjs`](../astro-typograf.config.mjs), подробный
разбор — в `CLAUDE.md`, раздел «Микротипографика».
...
| Исходники `.astro` | нет и не должно быть — держим их чистыми |
...
Скрипт печатает весь видимый русский текст, до которого селектор не дотянулся.
Норма — «Весь видимый текст покрыт селектором». Прогонять при добавлении
любого нового текстового блока (пункт уже есть в чек-листе раздела 7).
