...
### 1. Фотографии с тренингов

Блок-галерея с живыми кадрами с очных тренингов Дениса.

- [ ] Получить исходники фото от Дениса, отобрать 6–12 кадров.
- [ ] Положить в `src/assets/images/trainings/`, подключать **только** ES-импортом
      (не хардкодить `/pdca-prp/...`), прогнать через `<Image />` для WebP + lazy.
- [ ] Новый компонент `src/components/TrainingPhotos.astro`, вставить в
      `src/pages/index.astro`. Обязательно `relative z-10` + непрозрачный фон
      (раздел «Стэкинг и fixed-слои hero» в `CLAUDE.md`).
- [ ] Решить формат: сетка / слайдер. Если слайдер — переиспользовать логику
      `VideoReviews.astro`, а не писать вторую реализацию.

### 2. Логотипы компаний-клиентов

Лента логотипов организаций, с которыми работал Денис, — социальное
доказательство под блоком о тренере.

- [ ] Собрать список компаний и получить логотипы (нужен SVG или PNG ≥2×;
      растровые «скриншоты с сайта» не годятся).
- [ ] Уточнить у Дениса права на публикацию каждого логотипа.
- [ ] Компонент `src/components/Clients.astro`: моно-обработка (grayscale /
      единая высота), чтобы разнокалиберные логотипы не разваливали ритм.

### 3. Форма заявки

Целевое действие лендинга. Сейчас его нет вообще — страница заканчивается
отзывами.
...
- [ ] **Решить, куда уходят заявки.** Сайт статический на GitHub Pages, бэкенда
      ...
      serverless-функция с отправкой в Telegram. Это блокирующее решение,
      от него зависит вся вёрстка формы.
- [ ] Поля: имя, телефон или e-mail, компания, комментарий (минимум обязательных).
- [ ] Состояния: валидация, отправка, успех, ошибка сети. Без «немой» кнопки.
- [ ] Чекбокс согласия на обработку персональных данных со ссылкой на документ
      (см. п. 5) — без него форму запускать нельзя.
- [ ] Антиспам: honeypot-поле или капча провайдера.
- [ ] Якорь `#request` + ведущие на него CTA из hero и offer-блока.

### 4. Футер

- [ ] Компонент `src/components/Footer.astro`, последним в `index.astro`.
- [ ] Контакты Дениса, ссылки на соцсети, копирайт с годом.
- [ ] Блок юридических ссылок (п. 5) и реквизиты (ИП/самозанятый, ИНН) —
      уточнить у заказчика, что именно публикуем.
- [ ] Навигация-якоря по секциям страницы (опционально).
...
- [ ] Скан сертификата Euclides Coimbra: сейчас в `index.astro` заглушка,
      импорт закомментирован (строки 18–22) — файл в `src/assets/images/`,
