...
**Источник правды — [`docs/SPACING.md`](docs/SPACING.md).** Открыть ПЕРЕД
добавлением нового блока и перед любой правкой `py-*`, `mt-*`, `mb-*`, `gap-*`.

Коротко: отступ — это **роль**, а не число. Шесть токенов в `@theme`
(`--spacing-section`, `-section-lg`, `-block`, `-group`, `-gutter`, `-panel`)
дают и `var()`, и Tailwind-утилиты (`mt-block`, `gap-gutter`, `p-panel`).

| Ступень | Значение | Роль |
|---|---|---|
| `section` | `clamp(3rem, 6vw, 8rem)` | вертикаль секции на светлом фоне (`.section-py`) |
| `section-lg` | `clamp(4rem, 7vw, 9rem)` | вертикаль секции поверх фотопанорамы (`.section-py-lg`) |
| `block` | `clamp(2.5rem, 5vw, 4.5rem)` | шапка секции → контент; крупные блоки внутри |
| `group` | `clamp(2rem, 3.5vw, 3rem)` | подраздел внутри блока |
| `gutter` | `clamp(2.5rem, 4vw, 5rem)` | зазор между колонками grid'а |
| `panel` | `clamp(1.75rem, 3vw, 3rem)` | внутренние поля панели и карточек модалок |

Горизонталь — две ступени: `.section-px` (базовая) и `.section-px-wide`
(«ковровый» контент на lg+: диаграмма ПРП, лента логотипов). Ниже ступени
`group` работает обычная шкала Tailwind: eyebrow → H2 всегда `mb-6`,
H2 → лид `mt-8`, текст → подпись `mt-6`.

Поля контента модалок — всегда ступень `panel` (`p-panel` / `px-panel`),
одинаково у лонгрида, списка, цитаты и расшифровки. У полосы-шапки карточки
горизонталь та же, вертикаль меньше (`py-3`/`py-5`) — это край панели,
а не панель. Лайтбоксы сертификата и галереи полей не имеют вовсе.

**Запрещено:**
- свой `clamp()` для отступа в разметке (единственное исключение —
  `gap-x` колонки с иероглифом в `AboutTraining.astro`, оно закомментировано);
- фиксированный `py-16`/`mb-10` на границе секции или блока — секция обязана
  дышать вместе с вьюпортом;
- новая ступень «чуть больше существующей»;
- асимметричный `py` без комментария-обоснования;
- элемент, торчащий за поля секции: на мобильном он растягивает горизонтальную
  прокрутку всей страницы (`visibility: hidden` из потока не убирает).

Контрольный grep — должно остаться ровно одно попадание, то самое исключение:
...
- `img#sky-fixed` — небо-подложка во весь экран (`fixed inset-0 object-cover`),
  ...
  закреплена на всех брейкпоинтах, включая мобильный;
